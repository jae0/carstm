...
# categorize Strata
o = over(
  SpatialPoints( set[,c("lon", "lat")], sp::CRS(projection_proj4string("lonlat_wgs84")) ),
  spTransform(sppoly, sp::CRS(projection_proj4string("lonlat_wgs84")) )
...
fit = lm(
  formula = totwgt ~ StrataID:yr_factor - 1  ,
  data=set[ok,]
)
s = summary(fit)
AIC(fit)  # 106879
...
means = split_vector2matrix(
  ...
  if (0) {
    ...
    w = gsub("StrataID", "", w)
    w = gsub("yr_factor", "", w)
    sid = match( w[,1], as.character(sppoly$StrataID) )
...
out = means * sppoly$sa_strata_km2 / standardtow_sakm2
...
sppoly@data[,vn] = out[,"2017"] / standardtow_sakm2
...
fit = glm(
  formula = totno ~ StrataID:yr_factor - 1  ,
...
APS = expand.grid( StrataID=levels(set$StrataID), yr_factor=levels(set$yr_factor) )

kk = which( paste(APS$StrataID, APS$yr_factor) %in% unique(paste( set$StrataID, set$yr_factor)[ok] ))
...
RES$glm_poisson_totno = colSums(
  {out/standardtow_sakm2 * weight_year * sppoly$sa_strata_km2}[sppoly$strata_to_keep , ], na.rm=TRUE )
...
fit = glm(
  formula = totno ~ offset(log(data_offset)) + StrataID:yr_factor - 1 ,
  family=poisson(link="log"),
  data=set[ok,]
)
AIC(fit)  # 359785
...
RES$glm_poisson_totno_offset = colSums( {out * weight_year * sppoly$sa_strata_km2}[sppoly$strata_to_keep , ], na.rm=TRUE )
lines( glm_poisson_totno_offset ~ yr, data=RES, lty=5, lwd=4, col="red")


vn = "glm_poisson_totno_offset"
sppoly@data[,vn] = out[,"2010"]
brks = interval_break(X= sppoly[[vn]], n=length(p$mypalette), style="quantile")
spplot( sppoly, vn, col.regions=p$mypalette, main=vn, at=brks, sp.layout=p$coastLayout, col="transparent" )
