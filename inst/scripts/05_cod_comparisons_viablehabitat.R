...
sppoly = neighbourhood_structure( sppoly=sppoly, areal_units_strata_type="stratanal_polygons" )
...
# categorize Strata
o = over( SpatialPoints( set[,c("plon", "plat")], sp::CRS(projection_proj4string("lonlat_wgs84")) ), spTransform(sppoly, sp::CRS(projection_proj4string("lonlat_wgs84")) ) ) # match each datum to an area
...
RES$habitat_glm = colSums( {out * sppoly$sa_strata_km2 }[sppoly$strata_to_keep,], na.rm=TRUE ) /sum(sppoly$sa_strata_km2[sppoly$strata_to_keep]) # sa weighted average prob habitat
...
RES$habitat_gam = colSums( {out * sppoly$sa_strata_km2 }[sppoly$strata_to_keep,], na.rm=TRUE ) /sum(sppoly$sa_strata_km2[sppoly$strata_to_keep]) # sa weighted average prob habitat
...
RES$habitat_inla = colSums( {out * sppoly$sa_strata_km2 }[sppoly$strata_to_keep,], na.rm=TRUE ) /sum(sppoly$sa_strata_km2[sppoly$strata_to_keep]) # sa weighted average prob habitat
...
out = matrix(NA, nrow=length(sppoly$StrataID), ncol=length(p$yrs), dimnames=list( sppoly$StrataID, p$yrs) )
out[ cbind(APS$istrata, APS$iyr) ] = APS$mean
RES$habitat_strata_CAR_yr.yr_iid = colSums( {out * sppoly$sa_strata_km2 }[sppoly$strata_to_keep,], na.rm=TRUE ) /sum(sppoly$sa_strata_km2[sppoly$strata_to_keep]) # sa weighted average prob habitat
