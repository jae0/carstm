...
carstm_model = function( p=list(), data=NULL, sppoly =NULL, areal_units_fn=NULL, DS="redo", 
  ...
  if ( grepl("glm", carstm_modelengine) ) {
    ...
    out = carstm_model_glm( O=p, data=data, fn_fit=fn_fit,  compress=compress, compression_level=compression_level, ... ) 
  }

  if ( grepl("gam", carstm_modelengine) ) {
    # not a CAR but for comparison with no spatial random effect model
    out = carstm_model_gam( O=p, data=data, fn_fit=fn_fit,  compress=compress, compression_level=compression_level, ... ) 
  }
  ...
  if ( grepl("inla", carstm_modelengine) ) {
    out = carstm_model_inla( O=p, data=data, sppoly=sppoly, fn_fit=fn_fit, 
