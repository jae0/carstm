...
carstm_model_inla_retired = function(p, data=NULL, E=NULL, sppoly=NULL, region.id=NULL,
  ...
  O[["formula"]] = p$formula
  O[["formula_parsed"]] = fm = carstm_parse_formula(p$formula )
  ...
  if (is.null(data)) if (exists("data", O)) data = O$data
  setDF(data)
  ...
  if (grepl("space", p$aegis_dimensionality)) {
    data[,vnS0] = as.character( data[,vnS] ) # local copy
    data[,vnS] = match( data[,vnS0], O[[vnS]] )  # overwrite with numeric values that must match index of neighbourhood matrix
  }

  if ( grepl("year", p$aegis_dimensionality) | grepl("season", p$aegis_dimensionality) ) {
    if (any( grepl( vnT, p$formula )))  {
      O[[vnT]] = as.character( p$yrs )
      data[,vnT0] = as.character( data[,vnT] ) # a copy for internal matching 
      data[,vnT] = match( data[,vnT0], O[[vnT]] ) # convert to numeric (ie. a numeric factor)
    }
    # internal vars, for inla
    if (any( grepl( vnST, p$formula )))  data[,vnST] = data[,vnS]
    if (any( grepl( vnST, p$formula )))  data[,vnTS] = data[,vnT]
    # sub-annual time
    if (any( grepl( vnU, p$formula )))  {
      O[[vnU]] = as.character( p$dyears + diff(p$dyears)[1]/2)
      data[,vnU0] = as.character( data[,vnU] )  # a copy for internal matching 
      data[,vnU] = match( data[,vnU0], O[[vnU]] )  # convert to numeric (ie. a numeric factor)
    }
  }

  # on user scale
  ii = which(is.finite(data[ , vnY ]))
  
  mq = quantile( data[ ii, vnY ], probs=c(0.025, 0.5, 0.975) )

  O[["data_range"]] = c( 
    mean=mean(data[ ii, vnY ]), 
    sd=sd(data[ ii, vnY ]), 
    min=min(data[ ii, vnY ]), 
    max=max(data[ ii, vnY ]),  
    lb=mq[1], 
  ...
  if (exists("data_transformation", p)) data[, vnY]  = p$data_transformation$forward( data[, vnY] ) 

  # get hyper param scalings

  # temp Y var on link scale:
  yl = lnk_function( data[, vnY ] )   # necessary in case of log(0)
  ...
  if ( !is.null(fm$offset_variable) )  {
    ...
    obs = 1:nrow(data)
    if (exists("tag", data)) {
      obso = which(data$tag=="observations")
      if (length(obso) > 3) obs = obso
      obso = NULL
    }   
    
    ol = lnk_function( data[, vnO ])
  ...
  H = hyperparameters( sd(yl[ll] ), alpha=0.5, median(yl[ll], na.rm=TRUE) )  # sd slightly biased due to 0's being dropped
  ...
  if (redo_fit) {
    ...
    ellp[["formula"]] = p$formula
    ellp[["family"]] = p$family
    ...
    data = ellp[["data"]]  # rename for followup
    ellp = NULL; gc()
  ...
  if ("predictions"  %in% toget ) {
    ...
    if (!exists("tag", data)) data$tag="predictions" # force predictions for all data
    ...
    if (exists("marginals.fitted.values", fit)) {
      ...
      if (  p$aegis_dimensionality == "space" ) {
        ipred = which( data$tag=="predictions"  &  data[,vnS0] %in% O[[vnS]] )  # filter by S and T in case additional data in other areas and times are used in the input data
        ...
        matchfrom = list( space=data[ ipred, vnS0] ) 
        matchto = list( space=O[[vnS]] )
      ...
      if ( p$aegis_dimensionality == "space-year" ) {
        ipred = which( data$tag=="predictions" & data[,vnS0] %in% O[[vnS]] & data[,vnT0] %in% O[[vnT]] )
        g = fit$marginals.fitted.values[ipred]   
        if ( exists("offset_scale_revert", O) ) g = apply_generic( g, function(u) {inla.tmarginal( O$offset_scale_revert, u) } )    
        ...
        matchfrom = list( space=data[ipred,vnS0], time=data[ipred,vnT0] )
        matchto = list( space=O[[vnS]], time=O[[vnT]] )
      ...
      if ( p$aegis_dimensionality == "space-year-season" ) {
        ipred = which( data$tag=="predictions" & data[,vnS0] %in% O[[vnS]]  &  data[,vnT0] %in% O[[vnT]] &  data[,vnU0] %in% O[[vnU]])  # ignoring U == predict at all seassonal components ..
        g = fit$marginals.fitted.values[ipred]   
        if ( exists("offset_scale_revert", O) ) g = apply_generic( g, function(u) {inla.tmarginal( O$offset_scale_revert, u) } )    
        ...
        matchfrom = list( space=data[ipred, vnS0], time=data[ipred, vnT0], season=data[ipred, vnU0] )
