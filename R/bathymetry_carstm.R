  ...
  if ( DS=="carstm_inputs") {
    ...
    # reduce size
    if (p$carstm_inputs_aggregated) {
      M = bathymetry.db ( p=p, DS="aggregated_data", data_root_override=TRUE  )  # 16 GB in RAM just to store!
      names(M)[which(names(M)==paste(p$variabletomodel, "mean", sep=".") )] = p$variabletomodel

    } else {
      M = bathymetry.db ( p=p, DS="z.lonlat.rawdata", data_root_override=TRUE )  # 16 GB in RAM just to store!
