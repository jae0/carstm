...
```r
require(carstm)
toydata = carstm::toy_spatial_data()
...
#------------RCall session starts here:
# NOTE: to start RCall session, type $ ... and then run the following in R:
 
  require(carstm)
  toydata = carstm::toy_spatial_data() # (same) generate some spatial data
  attach(toydata)  # (data, wb)

  str(data)
  str(wb) 
 
# NOTE: to end RCall session, type <backspace> to escape back to julia
#------------RCall session ends here:


# Now that we are back in Julia, we can fetch the data from the RCall session:
@rget dat
@rget W

source_directory = "/home/jae/bio/carstm/" # alter this to location of car_functions.jl
...
auid = 1:nY  # not useful here as multiples in an au do not exist ...
...
~~~
  parameters      mean       std   naive_se      mcse         ess      rhat   ess_per_sec 
      Symbol   Float64   Float64    Float64   Float64     Float64   Float64       Float64 
       ...
       beta0   -0.4428    0.0818     0.0013    0.0029    703.1554    1.0028        2.1159
    betas[1]    1.0883    0.0629     0.0010    0.0016   1693.3683    1.0008        5.0956
    betas[2]    1.0453    0.0566     0.0009    0.0015   1750.5598    0.9998        5.2677
     sum_phi   -0.0000    0.0985     0.0016    0.0021   3180.2821    0.9997        9.5700
       sigma    0.4060    0.0509     0.0008    0.0014    991.7886    1.0027        2.9844
         rho    0.9223    0.0988     0.0016    0.0054    359.8538    1.0059        1.0829

recovers sigma =0.4
rho : nn when distance <= 1 .. exp(-0.1 *1) = 0.905 
...
o = sample(m, turing_sampler, n_samples); showall( summarize(o) )
  ...
  parameters      mean       std   naive_se      mcse       ess      rhat   es ⋯
      Symbol   Float64   Float64    Float64   Float64   Float64   Float64      ⋯
       alpha    0.9577    0.0457     0.0046    0.0090   19.0341    1.0173        0.0886
         tau    1.4950    0.4459     0.0446    0.0448   61.2655    0.9904        0.2851
     beta[1]    0.1343    0.3446     0.0345    0.0935    4.5427    1.1501        0.0211
     beta[2]    0.2585    0.0929     0.0093    0.0140   39.0110    1.0545        0.1815
      phi[1]    1.0993    0.4747     0.0475    0.0957   15.0951    1.0521        0.0702
 

m = turing_car_prec(D, W, X, log_offset, y, nX )   # 1 min 
o = sample(m, turing_sampler, n_samples); showall( summarize(o) )
 
       alpha    0.9530    0.0581     0.0058    0.0141   12.8247    1.0056        0.1802
         tau    1.5805    0.5428     0.0543    0.1020   22.2444    1.0252        0.3125
     beta[1]   -0.2075    0.3778     0.0378    0.0850   11.2222    1.0522        0.1577
     beta[2]    0.2415    0.0998     0.0100    0.0162   35.3120    0.9910        0.4961
      phi[1]    1.4373    0.4513     0.0451    0.0855   16.6786    1.0330        0.2343
...
o = sample(m, turing_sampler, n_samples); showall( summarize(o) )
     ...
     beta[1]   -0.2315    0.1318     0.0132    0.0369     5.7811    1.1113        0.2841
     beta[2]    0.3101    0.0947     0.0095    0.0203    22.4170    0.9923        1.1017
     sum_phi    0.0020    0.0552     0.0055    0.0052   106.7030    0.9900        5.2439
   tau_theta    4.7220    1.2700     0.1270    0.2202     4.6231    1.1546        0.2272
     tau_phi    1.4779    0.5100     0.0510    0.1036    20.0531    1.0215        0.9855
...
Continued in .. ![](notes_gaussian_process_comparisons.qmd) and ![](notes_carstm_julia.qmd)
...
NOTE: example seems broken 2023/06/22 .. (skip for now)
