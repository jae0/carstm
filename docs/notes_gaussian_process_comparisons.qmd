...
## 6. GP kernel ridge regression with Cholesky 
...
$s = L v + u$;  with $LL^T = \Sigma = K+\lambda 1$ 

and $v \sim N(0,1)$ and 

$u$ is the mean

$u =  X  \beta $ ; $ \beta = X^{T} \Sigma^{-1} y $ ; $\beta = X^{T} (K+\lambda 1)^{-1} y$ 
...
q = sample(m, NUTS(nadapt, target_accept_ratio, max_depth=10), iterations);
...
plot(vec(Xnew), vec(pred_mean), color="red", label="Posterior predictive mean")
scatter(vec(X), vec(y), color="grey", label="Data")
fill_between(vec(Xnew), vec(pred_upper), vec(pred_lower), color="red", alpha=0.2)
legend(loc="lower right")
title("GP Posterior predictive with 95% credible interval");
